...
const twilioentry = require('./googlevoiceentry.js')
...
describe('handler', () => {
	...
	it('responds with SIP dial for valid contact', (done) => {
		twilioentry.handler(base_context, {}, (err, result)=>{
			...
			expect(responseXml).to.not.contain(`action="${SIP_DIAL_ACTION}"`);
			done();
		});
	});

	it('hangs up for invalid contact', (done) => {
		nock.cleanAll();
		token_request = nock(NOCK_BASE_URL)
			.post('/oauth/token')
			.reply(200, {
		...
		twilioentry.handler(base_context, {}, (err, result)=>{
			...
			assert.isNotNull(result);
			let responseXml = result.response.toString();
			expect(responseXml).to.contain('<Hangup/>');
			expect(responseXml).to.not.contain(FORWARDING_NUMBER);
			done();
		});
	});
